...
def preprocess_data(w_size,problem_name):
    ...
    femfile_dir = osp.join(data_dir,"meshes","extend_base","small_10_base_10")
    geo_dir = osp.join(data_dir, "geo_models","small_10") 
    vtk_dir = osp.join(data_dir,"vtk", problem_name)
    ...
    ratio = np.abs(np.min(fem_file["vertices"][:,2]))/depth
    ...
    for i_deposit in range(deposit_pairs.shape[0]):
        vtk_path_00 = osp.join(vtk_dir, f"u_{(deposit_pairs[i_deposit,0]-1):05d}_active_{0:05d}.vtu")       
        vtk_path_01 = osp.join(vtk_dir, f"u_{(deposit_pairs[i_deposit,1]-1):05d}_active_{0:05d}.vtu")
        ...
        i_time_frame = i-1
        if i_time_frame > 10:
            for i_ele in range(i_time_frame-10,i_time_frame+1):
